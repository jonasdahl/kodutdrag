...
object Trees {
  ...
  case class VarDecl(tpe: TypeTree, id: Identifier, expr: ExprTree) extends Tree with Symbolic[VarSymbol]
  ...
  case class FunctionDecl(args: List[Formal], expr: ExprTree) extends ExprTree with Typed with Symbolic[FunctionSymbol] {}
  sealed case class Formal(tpe: TypeTree, id: Identifier) extends Tree with Symbolic[VarSymbol]
  ...
  case class FunctionType(args: List[TypeTree], ret: TypeTree) extends TypeTree
  ...
  case class FunctionCall(meth: ExprTree, args: List[ExprTree]) extends ExprTree with Symbolic[FunctionSymbol]
  ...
  case class Identifier(value: String) extends TypeTree with ExprTree with Symbolic[Symbol] {
    ...
    override def getType: Type = getSymbol match {
      case cs: ClassSymbol => TAnyRef(cs)
      case ms: MethodSymbol => sys.error("Requesting type of a method identifier.")
      case ms: GlobalScope => sys.error("Requesting type of global scope.")
      case vs: FunctionSymbol => vs.getType
      case vs: VarSymbol => vs.getType
    }
